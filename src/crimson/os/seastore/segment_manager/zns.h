...
namespace crimson::os::seastore::segment_manager::zns {
	...
	struct ZoneReport {
		...
		: hdr((struct blk_zone_report *)malloc(sizeof(struct blk_zone_report) + nr_zones * sizeof(struct blk_zone))){}
		~ZoneReport(){
			free(hdr);
		}
	...
	public:
		ZNSSegment(ZNSSegmentManager &man, segment_id_t i) ;
	...
	public:
		...
		uint64_t get_offset(paddr_t addr) {
			...
			(addr.segment.device_segment_id() * metadata.zone_size)) * 512 +
			addr.offset;
		}
