...
namespace crimson::os::seastore::segment_manager::zns{
  ...
  static seastar::future<size_t> get_zone_capacity(seastar::file &device, int zone_size, int nr_zones)
  {
    return seastar::do_with(
      blk_zone_range{},
      ZoneReport(nr_zones),
    ...
    size_t block_size)
  {
    logger().error(
      ...
      offset,
      bl.length());
    logger().error("BLOCK_SIZE: {}", block_size);
    ...
    bl.prepare_iov(&iov);
    logger().error("IOV len: {}: ", iov.size());
    for(auto it : iov){
      logger().error("IOV BUF LEN {}: ", it.iov_len);
    }
    return device.dma_write(
      offset,
      std::move(iov)
  ...
  bufferptr &bptr)
{
  ...
  ZNSSegmentManager::mkfs_ret ZNSSegmentManager::mkfs(segment_manager_config_t config){
        ...
        return maybe_create
          ...
          .safe_then([&](auto p){
            ...
            return device.ioctl(
              ...
              if (nr_zones == 0) {
                return seastar::make_exception_future<int>(std::system_error(std::make_error_code(std::errc::io_error)));
              }
              ...
              sb = make_metadata(config.meta, stat, zone_size, zone_capacity, nr_zones);
              metadata = sb;
  ...
  struct blk_zone_range make_range(segment_id_t id, size_t segment_size, size_t block_size, size_t first_segment_offset){
    logger().error("Segment size {}", segment_size);
  ...
  ZNSSegmentManager::open_ertr::future<SegmentRef> ZNSSegmentManager::open(segment_id_t id) {
    return seastar::do_with(
      blk_zone_range{},
      [=] (auto &range){
        range = make_range(id, metadata.zone_size, metadata.block_size, metadata.first_segment_offset);
        logger().error("SECTOR: {} NR_SECTORS: {}", range.sector, range.nr_sectors);
  ...
  blk_close_zone_ret blk_close_zone(seastar::file &device, blk_zone_range &range){
    return device.ioctl(
      BLKCLOSEZONE, 
      &range
    ).then_wrapped([=] (auto f) -> blk_open_zone_ret{
  ...
  ZNSSegmentManager::release_ertr::future<> ZNSSegmentManager::release(segment_id_t id) {
    return seastar::do_with(
      blk_zone_range{},
      [=] (auto &range){
        range = make_range(id, metadata.zone_size, metadata.block_size, metadata.first_segment_offset);
  ...
  ceph::bufferptr &out){
    ...
    if (addr.offset + len > metadata.zone_size) {
      logger().error(
    ...
    logger().error("IN READ: {}", get_offset(addr));
    return do_read(
      device,
      get_offset(addr),
      len,
    ...
    bool ignore_check)
  {
    ...
    return do_writev(device, get_offset(addr), std::move(bl), metadata.block_size);
  }
